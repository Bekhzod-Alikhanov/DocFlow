...
describe('redteam-latent-capability scenario', () => {
  it('validates against the schema', () => {
    expect(validateScenario(redteamLatentCapability)).toEqual({ ok: true, errors: [] })
  })

  it('foregrounds chapters 4 and 1', () => {
    const chapters = new Set(redteamLatentCapability.nodes.flatMap((n) => n.choices.map((c) => c.chapter)))
    expect(chapters.has(4)).toBe(true)
    expect(chapters.has(1)).toBe(true)
  })

  it('carries the cyber ~95% estimate caveat verbatim somewhere', () => {
    const caveats = redteamLatentCapability.nodes.flatMap((n) =>
  ...
  it('has no dominant path (the thesis property)', () => {
    expect(hasDominantPath(redteamLatentCapability)).toBe(false)
  })

  it('the suppress-and-recall / oral path wins legal safety but loses recurrence and remediation vs the two-track path', () => {
    const scored = scoreAllPaths(redteamLatentCapability)
