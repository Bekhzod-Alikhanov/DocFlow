...
const STRUCTURAL_SPECS: ParamSpec[] = [
  ...
  { id: 'gain', label: 'Logistic gain (f_doc steepness)', unit: 'dimensionless', default: 15, min: 1, max: 20, group: 'documentation', evidence_basis: ILLUSTRATIVE, source: SPEC_SRC, note: 'Steepness of the documentation-fraction sigmoid. Higher → sharper tipping threshold; it also deepens the culture fold. NO EVIDENCE FIXES THIS VALUE. At the default the model has two attractors; at low values it has one, varying smoothly. The two-equilibrium structure is therefore a CONSEQUENCE of this free parameter and not a result the model establishes — see RISKS.md R4 and the fold-boundary sweep.', advanced: true, tier: 'T4', whatWouldConstrainIt: 'Observed sharpness of the transition between documenting and not documenting, e.g. from a firm that crossed it.' },
  ...
  // --- v0.3.0 M3c: the remediation channel's admissibility (MODEL_v3_SPEC 3.2) ---
  { id: 'w_407', label: 'Remediation-record discoverability', unit: 'PD/channel', default: 0.5, min: 0, max: 2, group: 'documentation', evidence_basis: ILLUSTRATIVE, source: 'Fed. R. Evid. 407 excludes the remedial measure, not everything around it', note: 'Baseline discoverability pressure on Channel Three. Rule 407 governs ADMISSIBILITY, not discovery, so this term is a hedge rather than a shield -- see ADR/0004, which argues C3 should not carry a meaningful exposure reduction at all.', advanced: true, tier: 'T4', whatWouldConstrainIt: 'Coded outcomes of Rule 407 disputes over remediation records, scored for whether the remedial measure and the statements around it were excluded together or separately.', citationStatus: 'unverified' },
  { id: 'q_407', label: 'Share of Rule 407 protection that attaches', unit: '0-1', default: 0.5, min: 0, max: 1, group: 'documentation', evidence_basis: ILLUSTRATIVE, source: 'FRE 407 and its exceptions; CIRCIA s.681e non-admission analogy', note: 'How much of the 407 exclusion actually attaches, scaled by whatever non-admission scaffolding exists. Multiplied by safe_harbor_non_admission, which is the lever whose whole subject is this channel. Deliberately not a shield: 407 has broad exceptions and does not touch discovery.', advanced: true, tier: 'T4', whatWouldConstrainIt: 'Frequency with which 407 exclusion was granted in full versus pierced by an exception, in decided cases.' },
  { id: 'w_leak', label: 'Leakage -> remediation discoverability', unit: 'PD/leak', default: 0.6, min: 0, max: 2, group: 'documentation', evidence_basis: ILLUSTRATIVE, source: 'Playbook one-way valve; leaked causal language defeats the work-order framing', note: 'A remediation ticket carrying causal reasoning is no longer merely a work order, so leakage raises this channel discoverability directly -- on top of the waiver risk and independent admissions it already creates.', advanced: true, tier: 'T4', whatWouldConstrainIt: 'Audit of whether real remediation tickets carry causal language, and how such tickets fared in discovery.' },
  ...
  // --- culture ---
  { id: 'omega', label: 'Safety wins → culture', unit: 'culture/f_doc', default: 2.1, min: 0, max: 4, group: 'culture', evidence_basis: ILLUSTRATIVE, source: SPEC_SRC, note: 'Visible safety wins (documentation working, scaled by translation efficiency) raise culture — the strength of the R2 virtuous loop. NO EVIDENCE FIXES THIS VALUE. Together with `gain` it governs whether the culture fold exists at all, so the two-attractor structure enters the model here as an assumption rather than emerging from it (RISKS.md R4).', advanced: true, tier: 'T4', whatWouldConstrainIt: 'Longitudinal data on whether visible safety wins raise reporting rates.' },
  ...
  { id: 'c_inc_debt', label: 'Debt created per undocumented incident', unit: 'debt/incident', default: 1, min: 0, max: 3, group: 'documentation', evidence_basis: ILLUSTRATIVE, source: 'Structural unit conversion (VALIDATION.md V3.3)', note: 'Converts incidents leaving the undocumented stock into technical debt entering TD. Before v0.3.0 this conversion was implicit at 1: the same number was subtracted from a stock of incidents and added to a stock of debt, which is a dimensional error even though it is numerically harmless at unity. Defaulting to 1 keeps every trajectory identical while making the assumption visible and sweepable.', advanced: true, tier: 'T3', whatWouldConstrainIt: 'Structural conversion defining the debt index in terms of incidents. It is a definition of the index, not an empirical rate, so no observation constrains it — but a modeller who believes one undocumented incident creates more or less than one unit of debt can now say so.' },
  ...
  { id: 'disc_prob', label: 'Probability the record is reached in discovery', unit: '0-1', default: 0.5, min: 0, max: 1, group: 'exposure', evidence_basis: ILLUSTRATIVE, source: 'Fed. R. Civ. P. 26(b)(1) proportionality', note: 'Chance the factual record is actually produced. Channel One is discoverable BY DESIGN, so this is high and largely irreducible.', advanced: true, tier: 'T4', whatWouldConstrainIt: 'Production rates for engineering telemetry in comparable product-liability discovery.', citationStatus: 'unverified' },
  { id: 'xi_2', label: 'Unprotected analysis to exposure', unit: 'exposure/analysis', default: 0.9, min: 0, max: 3, group: 'exposure', evidence_basis: ILLUSTRATIVE, source: 'Playbook 1.2.2; In re Capital One (2020)', note: 'Exposure per privileged analysis TO THE EXTENT PRIVILEGE FAILS. Scaled by (1 - privilege survival).', advanced: true, tier: 'T4', whatWouldConstrainIt: 'Observed damage when a privilege claim over incident forensics is pierced.', citationStatus: 'unverified' },
  { id: 'xi_3', label: 'Remediation record -> PL exposure', unit: 'exposure/record', default: 0.02, min: 0, max: 0.3, group: 'exposure', evidence_basis: ILLUSTRATIVE, source: 'Fed. R. Evid. 407 limits admissibility at trial, not discovery (ADR/0004)', note: 'Exposure per remediation record, scaled by that channel discoverability. DELIBERATELY THE SMALLEST of the exposure conversions: 407 excludes the remedial measure itself, so a work order stripped of causal reasoning is weak evidence. It is NOT zero, which is what the model asserted before M3d -- 407 does nothing about discovery, and a channel with no exposure at all overstates the protection by more than the doctrine supports.', advanced: true, tier: 'T4', whatWouldConstrainIt: 'Cases where remediation records were produced in discovery and how much they contributed to a finding of fault despite a 407 objection.', citationStatus: 'unverified' },
